...
import {_render} from './zwe-object-row.render'
 export class ZweObjectRow extends ZweCore{
        
     constructor(){
        super();
        super.metadatakeyid=this.dataset.metadataKeyId;
//console.log(`ZweObjectRow^constructor^value of metadatakeyid:${this.metadatakeyid}`);
       }   
    
    connectedCallback(){ 
//console.log(`ZweObjectRow^connected callback^audit^entering connected callback: calling pre`);

//console.log(`ZweObjectRow^constructor^value of metadatakeyid:${this.metadatakeyid}`);
      this.connectedCallbackpre(this.metadatakeyid);
//console.log(`ZweObjectRow^connected callback^audit^entering connected callback: after calling pre`);
     } 
...
//console.log(`publishing zwe nav links data for elemtns ${itm.getId()} :: ${itm.name}`  );
     ...
     render(){
//console.log(`ZweObjectRow^render^audit^entering render`);
//console.log(`ZweObjectRow^render^audit^calling renderer of the layout render`);

      _render(this);  

     }  
}
customElements.define('zwe-object-row',ZweObjectRow)
